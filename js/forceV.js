...
function update() {  
  ...
  // var label = circlesg.selectAll(".mytext")
    // .data(nodes).enter().append("svg:a")
	// .append("text")
	// .attr('text-anchor', 'middle')
    // .attr("dy", ".9em")
    // .text(function(d) {return d.name +' '+ d.penaltyL +' '+ d.penaltyT})
	// .call(wrap, 120)
...
function node_dblclick(d) {
	...
	if (default_name == null) {}
	  ...
	  var t = Date.now();
	  var new_node = {x: m[0], y: m[1], name: default_name, group: 1, weight: 1, label:nn, Parent:null, Children:[], addTime:t, modifiedTime:null, penaltyT:1, penaltyL:0, dis:0};
...
var timeCounter = setInterval(timeElapse, f);
var globalThreshold = 0.6;
var nOverThreshold = [];
